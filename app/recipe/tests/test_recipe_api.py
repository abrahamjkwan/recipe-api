...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_partial_update_recipe(self):
        """Test partial updates on a recipe with patch"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        recipe.ingredients.add(sample_ingredient(user=self.user))
        new_tag = sample_tag(user=self.user, name='Wok Fried')

        payload = {
            'name': 'Stir-fried beef chow fun',
            'tags': [new_tag.id]
        }
    ...
    def test_full_update_recipe(self):
        """Test full updates on a recipe with put"""
        recipe = sample_recipe(user=self.user)
        recipe.tags.add(sample_tag(user=self.user))
        recipe.ingredients.add(sample_ingredient(user=self.user))
...
class RecipeImageUploadTests(TestCase):
    ...
    def test_filter_recipes_by_tags(self):
        ...
        recipe1 = sample_recipe(user=self.user, name='Spam fried rice')
        recipe2 = sample_recipe(user=self.user, name='Kimchi spam fried rice')
        tag1 = sample_tag(user=self.user, name='Kwan dishes')
        tag2 = sample_tag(user=self.user, name='Korean cuisine')
        recipe1.tags.add(tag1)
        recipe2.tags.add(tag2)
        recipe3 = sample_recipe(user=self.user, name='Instant ramen')
    ...
    def test_filter_recipes_by_ingredients(self):
        ...
        ingredient2 = sample_ingredient(user=self.user, name='kimchi')
        ingredient3 = sample_ingredient(user=self.user, name='spam')
        recipe1.ingredients.add(ingredient1)
        recipe1.ingredients.add(ingredient3)
        recipe2.ingredients.add(ingredient2)
        recipe2.ingredients.add(ingredient3)
        recipe3 = sample_recipe(user=self.user, name='Tuna Dip')

        res = self.client.get(
            RECIPES_URL,
            {'ingredients': f'{ingredient1.id}, {ingredient2.id}, {ingredient3.id}'}
        )
